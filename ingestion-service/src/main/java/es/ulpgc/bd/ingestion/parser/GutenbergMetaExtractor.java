package es.ulpgc.bd.ingestion.parser;
...
public class GutenbergMetaExtractor {
    ...
    public Meta extract(String text, GutenbergSplitter.Markers markers) {
        ...
        int preEnd = Math.max(0, Math.min(markers != null && markers.start != null ? markers.start.idx : full.length(), 20000));
        String pre = full.substring(0, preEnd);
        ...
        if ((author == null || author.isBlank()) && !byVals.isEmpty()) {
            ...
            for (String raw : byVals) {
                ...
                int uc = 0;
                for (String tkn : kept) {
                    if (!PARTICLES.contains(tkn.toLowerCase(Locale.ROOT)) && (tkn.matches("^[A-Z]\\.$") || Character.isUpperCase(tkn.charAt(0)))) uc++;
                }
